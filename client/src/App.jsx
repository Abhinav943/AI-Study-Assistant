          ...
          return (
            <span
              ...
              dangerouslySetInnerHTML={{
                __html: katex.renderToString(math, { displayMode: isBlock, throwOnError: false }),
              }}
            />
          );
        }
...
export default function App() {
  ...
  const resetSession = useCallback(() => {
    setData([]); setCurrentIndex(0);
    setFlashPhase('fact'); setSelectedFlashAnswer('');
    setSelectedAnswers([]); setTypedAnswer('');
    setShowExplanation(false); setIsCurrentCorrect(false);
    setScore(0); setWrongItems([]); setCorrectItems([]);
  ...
  const generateContent = async () => {
    ...
    try {
      ...
      if (!items.length) throw new Error('AI returned no items. Please try again.');
      setData(items);
      setStatus('studying');
    } catch (e) {
  ...
  const checkQuizAnswer = () => {
    ...
    let correct = false;
    if (q.type === 'single') {
      correct = selectedAnswers.length === 1 && q.correctAnswers.includes(selectedAnswers[0]);
      ...
      correct = q.correctAnswers.some(a => a.trim().toLowerCase() === typedAnswer.trim().toLowerCase());
    }
  ...
  const fetchReview = async () => {
    ...
    try {
      const res = await fetch('http://localhost:3000/api/review', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ total: data.length, score, wrongItems, correctItems, mode }),
      });
      const json = await res.json();
  ...
  const startOver = () => { resetSession(); setNotes(''); setStatus('idle'); };
  ...
  return (
    <div className="app-container">
      <header className="header">
        ...
        <p className="app-sub">AI-powered flashcards & quizzes from your notes</p>
      </header>
      ...
      <main className="glass-panel main-panel">
          ...
          <div className="input-section">
            ...
            <div className="config-grid">
              <div className="config-item">
                ...
                <select id="mode-sel" value={mode} onChange={e => setMode(e.target.value)}>
              ...
              <div className="config-item">
                ...
                <input id="count-inp" type="number" min="1" max="20" value={count} onChange={e => setCount(+e.target.value)} />
              </div>
              {mode === 'quiz' && (
                <div className="config-item">
                  <label htmlFor="qtype-sel">Question Type</label>
                  <select id="qtype-sel" value={quizType} onChange={e => setQuizType(e.target.value)}>
            ...
            <button id="generate-btn" className="btn btn-primary" onClick={generateContent} disabled={status === 'loading' || !notes.trim()}>
              {status === 'loading' ? (
        ...
        {status === 'studying' && mode === 'flashcard' && currentItem && (
          <div className="study-section">
            {showXpPop && <XpPop amount={xpPopAmount} onDone={() => setShowXpPop(false)} />}
            <HUD xp={xp} streak={streak} total={data.length} current={currentIndex} />

            <div className={`flashcard ${flashPhase !== 'fact' ? 'flipped' : ''}`} onClick={() => flashPhase === 'fact' && setFlashPhase('question')}>
              <div className="flashcard-inner">
                ...
                <div className="flashcard-face flashcard-back glass-panel">
                  ...
                  <p className="card-question"><MathText text={currentItem.question} /></p>
                  ...
                  <div className="options-grid">
                    {currentItem.options.map((opt, i) => {
                      ...
                      if (flashPhase === 'result') {
                        ...
                        else if (opt === selectedFlashAnswer) cls += ' opt-wrong';
                        else cls += ' opt-dim';
                      ...
                      return (
                        <button
                          ...
                          id={`flash-opt-${i}`}
                          className={cls}
                          onClick={e => { e.stopPropagation(); handleFlashAnswer(opt); }}
                  ...
                  {flashPhase === 'result' && (
                    <div className={`inline-explanation ${isCurrentCorrect ? 'exp-correct' : 'exp-wrong'}`}>
                      <span className="result-chip">{isCurrentCorrect ? '✓ Correct' : '✕ Incorrect'}</span>
                      <p className="exp-text"><MathText text={currentItem.explanation} /></p>
                      <button
                        id="flash-next-btn"
                        className="btn btn-primary btn-sm"
                        onClick={e => { e.stopPropagation(); moveToNext(isCurrentCorrect); }}
                      >
                        {currentIndex < data.length - 1 ? 'Next Card →' : 'View Results'}
                      </button>
                    </div>
                  )}
                </div>
        ...
        {status === 'studying' && mode === 'quiz' && currentItem && (
          <div className="study-section">
            ...
            <div className="quiz-card glass-panel">
              <p className="quiz-question"><MathText text={currentItem.question} /></p>

              <div className="options-list">
                {currentItem.type === 'single' && currentItem.options.map((opt, i) => {
                  let cls = 'opt-btn opt-wide';
                  if (showExplanation) {
                    if (currentItem.correctAnswers.includes(opt)) cls += ' opt-correct';
                    else if (selectedAnswers.includes(opt)) cls += ' opt-wrong';
                    else cls += ' opt-dim';
                  } else if (selectedAnswers.includes(opt)) cls += ' opt-selected';
                  return (
                    <button key={i} id={`q-opt-${i}`} className={cls} onClick={() => setSelectedAnswers([opt])} disabled={showExplanation}>
                      <span className="opt-letter">{String.fromCharCode(65 + i)}</span>
                      <MathText text={opt} />
                    </button>
                  );
                ...
                {currentItem.type === 'multi' && currentItem.options.map((opt, i) => {
                  let cls = 'opt-btn opt-wide opt-multi';
                  if (showExplanation) {
                    if (currentItem.correctAnswers.includes(opt)) cls += ' opt-correct';
                    else if (selectedAnswers.includes(opt)) cls += ' opt-wrong';
                    else cls += ' opt-dim';
                  } else if (selectedAnswers.includes(opt)) cls += ' opt-selected opt-multi-sel';
                  return (
                    <button key={i} id={`q-opt-${i}`} className={cls}
                      onClick={() => setSelectedAnswers(prev => prev.includes(opt) ? prev.filter(a => a !== opt) : [...prev, opt])}
                      disabled={showExplanation}>
                      <span className="opt-letter">{selectedAnswers.includes(opt) ? '■' : '□'}</span>
                      <MathText text={opt} />
                    </button>
                  );
                })}
                ...
                {currentItem.type === 'typed' && (
                  <input id="typed-ans" className="typed-input" type="text" placeholder="Type your answer…"
                    value={typedAnswer} onChange={e => setTypedAnswer(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && !showExplanation && typedAnswer.trim() && checkQuizAnswer()}
                    disabled={showExplanation} />
                )}
              </div>
            </div>
            ...
            {!showExplanation ? (
              ...
              <div className={`explanation-panel glass-panel ${isCurrentCorrect ? 'exp-correct' : 'exp-wrong'}`}>
                <div className="exp-header">
                  <span className={`result-chip ${isCurrentCorrect ? 'chip-ok' : 'chip-err'}`}>
                    {isCurrentCorrect ? '✓ Correct' : '✕ Incorrect'}
                  </span>
                  {!isCurrentCorrect && (
                    <p className="correct-answer-line">
                      <strong>Correct:</strong> {currentItem.correctAnswers.join(' / ')}
                    </p>
                  )}
                </div>
                <div className="exp-body"><MathText text={currentItem.explanation} /></div>
                <button id="next-btn" className="btn btn-primary btn-sm" onClick={advanceAfterQuiz}>
        ...
        {status === 'summary' && (
          <div className="summary-section">
            ...
            <div className="summary-actions">
              <button id="review-btn" className="btn btn-primary" onClick={fetchReview}>
                Get Performance Review
              </button>
              {wrongItems.length > 0 && (
                <button id="retry-btn" className="btn btn-secondary"
                  onClick={() => {
                    setData(wrongItems); setCurrentIndex(0);
                    setFlashPhase('fact'); setSelectedFlashAnswer('');
                    setWrongItems([]); setCorrectItems([]); setScore(0); setStreak(0);
                    resetQuizState(); setStatus('studying');
                  }}>
                  Re-test Missed Items
                </button>
              )}
              <button id="start-over-btn" className="btn btn-ghost" onClick={startOver}>
                Start New Session
              </button>
            </div>
          </div>
        ...
        {status === 'review' && reviewData && (
          <div className="review-section">
            <div className="review-header">
              <span className={`grade-badge grade-${reviewData.grade}`}>{reviewData.grade}</span>
            ...
            <div className="review-grid">
              ...
              <div className="review-card glass-panel">
                <h3 className="review-card-title err-title">△ Areas to Improve</h3>
                <ul>
            ...
            <div className="summary-actions">
              {wrongItems.length > 0 && (
                <button id="retry-from-review-btn" className="btn btn-primary"
                  onClick={() => {
                    setData(wrongItems); setCurrentIndex(0);
                    setFlashPhase('fact'); setSelectedFlashAnswer('');
                    setWrongItems([]); setCorrectItems([]); setScore(0); setStreak(0);
              ...
              <button id="new-session-btn" className="btn btn-ghost" onClick={startOver}>Start New Session</button>
            </div>
          </div>
        )}
